...
What makes this process reliable for attackers is determinism. Blockchain execution is fully predictable. There is no randomness in how arithmetic behaves, and the same transaction will produce the same result on every [node](https://www.nervos.org/knowledge-base/difference_between_miner_full_node). Once an attacker understands the arithmetic rules, exploitation becomes a matter of precise calculation rather than guesswork.
