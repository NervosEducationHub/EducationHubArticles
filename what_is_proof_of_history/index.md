...
In decentralized systems, however, time becomes a surprisingly complex challenge. [Nodes](https://www.nervos.org/knowledge-base/difference_between_miner_full_node) are geographically distributed, local clocks drift, and there is no universally trusted time source. As a result, most blockchains rely on constant communication between nodes to agree on what happened and in which order. This process works, but it is slow and resource-intensive.
...
Solana’s consensus mechanism, known as Tower BFT (Byzantine Fault Tolerant), is built on top of Proof-of-Stake and relies heavily on the ordered structure provided by PoH. Validators vote on blocks based on their position in the PoH sequence, allowing for rapid [finality](https://www.nervos.org/knowledge-base/What_is_finality_crypto) and reduced rollback risk. In this way, PoH directly enhances both performance and reliability.
