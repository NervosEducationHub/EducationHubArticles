...
Once the channel is established, the participants exchange updated commitment transactions that represent the evolving balance distribution between them. These updates happen entirely off-chain, allowing for instant payments and negligible fees. Only when the channel is closed does the final [state](https://www.nervos.org/knowledge-base/state_and_state_change) get broadcast to the Bitcoin network.
