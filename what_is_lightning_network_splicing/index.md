...
Because these updates happen off-chain, the network can process transactions far more quickly than Bitcoin’s base layer, where blocks are produced roughly every ten minutes. Each payment simply modifies the internal [state](https://www.nervos.org/knowledge-base/state_and_state_change) of the channel without touching the blockchain. Only when the channel closes is the final balance broadcast and recorded permanently onchain.
