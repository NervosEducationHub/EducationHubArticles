...
要理解 Nervos DAO 是如何工作的，你必须先要知道它是按周期工作的，每个周期都是 180 个 epoch。在 Nervos 网络上，每个 epoch 大约是 4 小时。所以我们可以计算出每个周期大约持续 30 天。这意味着，如果你在 Nervos DAO 中存入资金，最短存入时间大约为 30 天，即一个周期。

这个周期在 [Neuron](https://docs.nervos.org/docs/basics/guides/crypto wallets/neuron/) 界面上用指示条表示，如下图所示：
...
虽然在 Nervos DAO 中进行存款并没有什么特别的困难，但提款过程往往会产生很多疑问，因为它由以下两个步骤组成：

* **第 1 步：**初始提款交易，也叫提款请求

你可以随时将此交易发送到 Nervos DAO 以表明你的提现请求。如果要这样做，最好等到补偿周期位于指示条的绿色区域以最大化 Nervos DAO 奖励。请注意，在补偿周期结束前请求提款，你的资金将停止产生奖励，但会保持锁定状态，直到整个周期结束。
...
如果你在当前周期内没有提出提款申请，资金将自动转入下一个周期。  
<br>  
...
* **第 2 步：**最终的提款交易，也叫解锁存款 

这一步的交易用于提取你的 CKB 本金以及累积的 Nervos DAO 奖励。你可以在周期结束时执行这一步，即当箭头到达指示条的末尾时，前提是你之前已完成第 1 步的提款请求。
...
- 由于存款周期为 30 天，你可以在 10 月的任何一天发起提款请求，这是第 1 步（请记住，建议在指示条的绿色区域执行此步骤，从二最大化奖励），但存款的解锁，即第 2 步，要到 11 月 1 日后才能操作。
- 如果你没有在周期结束前发起提款请求，在我们的例子中是 11 月 1 日，那么新的 30 天存款周期将自动开始，你将无法在 12 月 1 日之前解锁存款，并且为了解锁，你必须在 11 月的某个时候发起提款请求。
...
是的。Nervos DAO 的目的之一是为 DAO 存款者创建类似比特币的硬顶。作为长期持有者，你总是获得相同百分比的新增供应，但新增供应本身的速度正在下降。这就是为什么 APC 这个数值实际上随着时间的推移而下降。
...
是的，Nervos DAO 的工作原理与钱包无关，无论是 Neuron、Portal Wallet、CKBull 还是其他任何钱包。唯一的区别是不同钱包的用户界面展示 Nervos DAO 的页面不同。

* **我应该保留多少 CKB 才能提款？**

由于 Cell 模型，钱包里必须至少有 61 个 CKB 才能执行 Nervos DAO 的提款操作。 要了解 Cell 模型和 Nervos DAO 的工作原理，你可以阅读[这篇文章](https://medium.com/nervosnetwork/understanding-the-nervos-dao-and-cell-model-d68f38272c24)。
...
* 你的 Neuron 版本太旧了。建议下载可用的 Neuron 最新版。你可以在 Neuron 中单击帮助 — 检查更新来执行此操作。
* 数据库已损坏。你可以在工具中，点击清除所有同步数据，来清理缓存。
